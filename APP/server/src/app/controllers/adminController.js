...
const apoAprovado = async (req, res) => {
    ...
    try{
        ...
        const updatedApoiador = await Apoiador.findOneAndUpdate({_id:id}, avaliar)
...
const atualizarEvento = async (req, res) => {
    ...
    try{
        ...
        const updatedEvento = await Evento.findOneAndUpdate({_id:id}, evento)
...
const deletarEvento = async (req,res) => {
    ...
    try{
        ...
        res.status(500).json({message: 'Não foi possível encontrar o evento.'})
    
    }
    
}

module.exports = {cadastroAdmin, exibirAdmin, atualizarAdmin, exibirForm, apoAprovado, apoNegado, cadastroEvento, mostrarEventos, atualizarEvento, deletarEvento}